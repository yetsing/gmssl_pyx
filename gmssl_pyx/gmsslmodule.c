...
static PyObject *gmsslext_sm2_key_generate(PyObject *self,
  ...
  if (ret != GMSSL_INNER_OK) {
    PyErr_SetString(GmsslInnerError,
                    "libgmssl inner error in sm2_key_generate");
    return NULL;
  }
...
static PyObject *gmsslext_sm2_encrypt(PyObject *self, PyObject *args,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "y#y#", kwlist, &public_key,
                                   &key_length, &plaintext, &text_length)) {
    return NULL;
  }
  if (key_length != 64) {
  ...
  if (text_length < SM2_MIN_PLAINTEXT_SIZE ||
      text_length > SM2_MAX_PLAINTEXT_SIZE) {
  ...
  ret = sm2_encrypt(&sm2_key, (uint8_t *)plaintext, text_length, ciphertext,
                    (size_t *)&outlen);
  if (ret != GMSSL_INNER_OK) {
...
static PyObject *gmsslext_sm2_decrypt(PyObject *self, PyObject *args,
                                      PyObject *keywds) {
  SM2_KEY sm2_key;
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "y#y#", kwlist, &private_key,
                                   &key_length, &ciphertext, &text_length)) {
    return NULL;
  }
  if (key_length != 32) {
  ...
  if (text_length < SM2_MIN_CIPHERTEXT_SIZE ||
      text_length > SM2_MAX_CIPHERTEXT_SIZE) {
  ...
  ret = sm2_decrypt(&sm2_key, (uint8_t *)ciphertext, text_length, plaintext,
                    (size_t *)&outlen);
  if (ret != GMSSL_INNER_OK) {
...
static PyObject *gmsslext_sm2_sign_sm3_digest(PyObject *self, PyObject *args,
                                              PyObject *keywds) {
  SM2_KEY sm2_key;
  const char *private_key;
  Py_ssize_t key_length;
...
static PyObject *gmsslext_sm2_verify_sm3_digest(PyObject *self, PyObject *args,
                                                PyObject *keywds) {
  SM2_KEY sm2_key;
  const char *public_key;
  Py_ssize_t key_length;
  ...
  // sm2_verify_sm3_digest(public_key: bytes, digest: bytes, signature: bytes)
  // -> bool
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "y#y#y#", kwlist, &public_key,
                                   &key_length, &digest, &digest_length, &sig,
                                   &siglen)) {
    return NULL;
  }
  if (key_length != 64) {
...
static PyObject *gmsslext_sm2_sign(PyObject *self, PyObject *args,
                                   PyObject *keywds) {
  const char *private_key;
  ...
  int ret, ok;
  static char *kwlist[] = {"private_key", "public_key", "message", "signer_id",
                           NULL};

  // sm2_sign(private_key: bytes, public_key: bytes, message: bytes, signer_id:
  // t.Optional[bytes] = b'1234567812345678') -> bytes:
  ok = PyArg_ParseTupleAndKeywords(args, keywds, "y#y#y#|O", kwlist,
                                   &private_key, &private_key_length,
                                   &public_key, &public_key_length, &message,
                                   &message_length, &signer_id_obj);
  if (!ok) {
    return NULL;
  }
...
static PyObject *gmsslext_sm2_verify(PyObject *self, PyObject *args,
                                     PyObject *keywds) {
  const char *public_key;
  Py_ssize_t public_key_length;
  const char *message;
  ...
  PyObject *signer_id_obj = NULL;
  static char *kwlist[] = {"public_key", "message", "signature", "signer_id",
                           NULL};
  int ret, ok;
  ...
  ok = PyArg_ParseTupleAndKeywords(
      args, keywds, "y#y#y#|O", kwlist, &public_key, &public_key_length,
      &message, &message_length, &signature, &signature_length, &signer_id_obj);
  if (!ok) {
    return NULL;
  }
  const char *signer_id = NULL;
  ...
  if (ret != GMSSL_INNER_OK) {
    PyErr_SetString(GmsslInnerError,
                    "libgmssl inner error in sm2_verify_update");
    return NULL;
  }
...
static PyObject *gmsslext_sm3_hash(PyObject *self, PyObject *args,
                                   PyObject *keywds) {
  const char *message;
  Py_ssize_t message_length;
  int ok;
  ...
  ok = PyArg_ParseTupleAndKeywords(args, keywds, "y#", kwlist, &message,
                                   &message_length);
  if (!ok) {
    return NULL;
  }
...
static PyObject *gmsslext_sm3_hmac(PyObject *self, PyObject *args,
                                   PyObject *keywds) {
  const char *message;
  Py_ssize_t message_length;
  const char *key;
  ...
  ok = PyArg_ParseTupleAndKeywords(args, keywds, "y#y#", kwlist, &key,
...
static PyObject *gmsslext_sm3_kdf(PyObject *self, PyObject *args,
                                  PyObject *keywds) {
  const char *key;
  Py_ssize_t key_length;
  ...
  ok = PyArg_ParseTupleAndKeywords(args, keywds, "y#k", kwlist, &key,
                                   &key_length, &outlen);
  if (!ok) {
    return NULL;
  }
...
static PyObject *gmsslext_sm4_cbc_padding_encrypt(PyObject *self,
                                                  PyObject *args,
                                                  PyObject *keywds) {
  const char *key;
  Py_ssize_t key_length;
  ...
  ok = PyArg_ParseTupleAndKeywords(args, keywds, "y#y#y#", kwlist, &key,
                                   &key_length, &iv, &iv_length, &plaintext,
                                   &plaintext_length);
  if (!ok) {
    return NULL;
  }
...
static PyObject *gmsslext_sm4_cbc_padding_decrypt(PyObject *self,
                                                  PyObject *args,
                                                  PyObject *keywds) {
  const char *key;
  Py_ssize_t key_length;
  ...
  ok = PyArg_ParseTupleAndKeywords(args, keywds, "y#y#y#", kwlist, &key,
                                   &key_length, &iv, &iv_length, &ciphertext,
                                   &ciphertext_length);
  if (!ok) {
    return NULL;
  }
...
static PyObject *gmsslext_sm4_ctr_encrypt(PyObject *self, PyObject *args,
                                          PyObject *keywds) {
  const char *key;
  Py_ssize_t key_length;
  const char *ctr;
  ...
  ok = PyArg_ParseTupleAndKeywords(args, keywds, "y#y#y#", kwlist, &key,
                                   &key_length, &ctr, &ctr_length, &plaintext,
                                   &plaintext_length);
  if (!ok) {
    return NULL;
  }
...
static PyObject *gmsslext_sm4_ctr_decrypt(PyObject *self, PyObject *args,
                                          PyObject *keywds) {
  const char *key;
  Py_ssize_t key_length;
  const char *ctr;
  ...
  ok = PyArg_ParseTupleAndKeywords(args, keywds, "y#y#y#", kwlist, &key,
                                   &key_length, &ctr, &ctr_length, &ciphertext,
                                   &ciphertext_length);
  if (!ok) {
    return NULL;
  }
...
static PyObject *gmsslext_sm4_gcm_encrypt(PyObject *self, PyObject *args,
                                          PyObject *keywds) {
  int ok;
  const char *key;
  Py_ssize_t key_length;
  ...
  // sm4_gcm_encrypt(key: bytes, iv: bytes, aad: bytes, plaintext: bytes) ->
  // t.Tuple[bytes, bytes]
  ok = PyArg_ParseTupleAndKeywords(args, keywds, "y#y#y#y#", kwlist, &key,
                                   &key_length, &iv, &iv_length, &aad,
                                   &aad_length, &plaintext, &plaintext_length);
  if (!ok) {
    return NULL;
  }
  if (key_length != SM4_KEY_SIZE) {
  ...
  sm4_set_encrypt_key(&sm4_key, (uint8_t *)key);
  int ret = sm4_gcm_encrypt(&sm4_key, (uint8_t *)iv, iv_length, (uint8_t *)aad,
  ...
  PyObject *obj =
      Py_BuildValue("y#y#", out, plaintext_length, tag, (Py_ssize_t)16);
  PyMem_RawFree(out);
  return obj;
}

static PyObject *gmsslext_sm4_gcm_decrypt(PyObject *self, PyObject *args,
                                          PyObject *keywds) {
  int ok;
  const char *key;
  Py_ssize_t key_length;
  ...
  // sm4_gcm_decrypt(key: bytes, iv: bytes, aad: bytes, ciphertext: bytes, tag:
  // bytes) -> bytes
  ok = PyArg_ParseTupleAndKeywords(
      args, keywds, "y#y#y#y#y#", kwlist, &key, &key_length, &iv, &iv_length,
  ...
  sm4_set_encrypt_key(&sm4_key, (uint8_t *)key);
  int ret =
      sm4_gcm_decrypt(&sm4_key, (uint8_t *)iv, iv_length, (uint8_t *)aad,
...
static PyObject *gmsslext_rand_bytes(PyObject *self, PyObject *args,
                                     PyObject *keywds) {
  int ok, n, ret;
...
PyMODINIT_FUNC PyInit_gmsslext(void) {
  ...
  InvalidValueError =
      PyErr_NewException("gmsslext.InvalidValueError", GmsslInnerError, NULL);

...
static int Custom_init(CustomObject *self, PyObject *args, PyObject *keywds) {
  static char *kwlist[] = {"first", "last", "number", NULL};
  PyObject *first = NULL, *last = NULL, *tmp;

  if (!PyArg_ParseTupleAndKeywords(args, keywds, "|OOi", kwlist, &first, &last,
                                   &self->number))
    return -1;
...
PyTypeObject CustomType = {
    PyVarObject_HEAD_INIT(NULL, 0).tp_name = "gmsslext.Custom",
...
static PyObject *SM9PrivateKey_new(PyTypeObject *type, PyObject *args,
                                   PyObject *kwds) {
...
static int SM9PrivateKey_init(SM9PrivateKeyObject *self, PyObject *args,
                              PyObject *kwds) {
...
static PyObject *SM9PrivateKey_from_der(PyTypeObject *type, PyObject *args,
                                        PyObject *keywds) {
  const char *data;
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "y#", kwlist, &data,
                                   &data_length)) {
    return NULL;
  }
  SM9PrivateKeyObject *self =
...
static PyObject *SM9PrivateKey_to_der(SM9PrivateKeyObject *self,
  ...
  if (ret != GMSSL_INNER_OK) {
    PyErr_SetString(GmsslInnerError,
                    "libgmssl inner error in sm9_enc_key_to_der");
    return NULL;
  }
...
static PyObject *SM9PrivateKey_decrypt_from_der(PyTypeObject *type,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "sy#", kwlist, &password,
                                   &data, &data_length)) {
    return NULL;
  }
  SM9PrivateKeyObject *self =
...
static PyObject *SM9PrivateKey_encrypt_to_der(SM9PrivateKeyObject *self,
  ...
  if (ret != GMSSL_INNER_OK) {
    PyErr_SetString(GmsslInnerError,
                    "libgmssl inner error in sm9_enc_key_info_encrypt_to_der");
    return NULL;
  }
  return Py_BuildValue("y#", (char *)buf, (Py_ssize_t)len);
}
...
static PyObject *SM9PrivateKey_decrypt_from_pem(PyTypeObject *type,
  ...
  if (ret != GMSSL_INNER_OK) {
    ...
    PyErr_SetString(
        GmsslInnerError,
        "libgmssl inner error in sm9_enc_key_info_decrypt_from_pem");
    return NULL;
  }
  fclose(fp);
  return (PyObject *)self;
...
static PyObject *SM9PrivateKey_encrypt_to_pem(SM9PrivateKeyObject *self,
  ...
  if (ret != GMSSL_INNER_OK) {
    fclose(fp);
    PyErr_SetString(GmsslInnerError,
                    "libgmssl inner error in sm9_enc_key_info_encrypt_to_pem");
    return NULL;
  }
  fclose(fp);
  Py_RETURN_NONE;
...
static PyObject *SM9PrivateKey_decrypt(SM9PrivateKeyObject *self,
  ...
  ret =
      sm9_decrypt(&self->key, identity, identity_length, (uint8_t *)ciphertext,
                  ciphertext_length, (uint8_t *)plaintext, &c_plaintext_length);
  if (ret != GMSSL_INNER_OK) {
...
PyTypeObject GmsslextSM9PrivateKeyType = {
    PyVarObject_HEAD_INIT(NULL, 0).tp_name = "gmsslext.SM9PrivateKey",
...
static PyObject *SM9MasterPublicKey_new(PyTypeObject *type, PyObject *args,
                                        PyObject *keywds) {
...
static int SM9MasterPublicKey_init(SM9MasterPublicKeyObject *self,
                                   PyObject *args, PyObject *keywds) {
  int ok;
  static char *kwlist[] = {NULL};
  // 不接受任何参数
...
static PyObject *SM9MasterPublicKey_to_der(SM9MasterPublicKeyObject *self,
                                           PyObject *Py_UNUSED(ignored)) {
  // to_der(self) -> bytes
  uint8_t buf[1024];
  uint8_t *p = buf;
  ...
  if (ret != GMSSL_INNER_OK) {
    PyErr_SetString(GmsslInnerError,
                    "libgmssl inner error in sm9_enc_master_public_key_to_der");
    return NULL;
  }
  return Py_BuildValue("y#", (char *)buf, (Py_ssize_t)len);
}
...
static PyObject *SM9MasterPublicKey_from_pem(PyTypeObject *type, PyObject *args,
  ...
  if (ret != GMSSL_INNER_OK) {
    ...
    PyErr_SetString(
        GmsslInnerError,
        "libgmssl inner error in sm9_enc_master_public_key_from_pem");
    return NULL;
  }
  fclose(fp);
  return (PyObject *)self;
...
static PyObject *SM9MasterPublicKey_to_pem(SM9MasterPublicKeyObject *self,
  ...
  if (ret != GMSSL_INNER_OK) {
    fclose(fp);
    PyErr_SetString(GmsslInnerError,
                    "libgmssl inner error in sm9_enc_master_public_key_to_pem");
    return NULL;
  }
  fclose(fp);
  Py_RETURN_NONE;
...
static PyObject *SM9MasterPublicKey_encrypt(SM9MasterPublicKeyObject *self,
                                            PyObject *args, PyObject *keywds) {
...
PyTypeObject GmsslextSM9MasterPublicKeyType = {
    PyVarObject_HEAD_INIT(NULL, 0).tp_name = "gmsslext.SM9MasterPublicKey",
...
static PyObject *SM9MasterKey_new(PyTypeObject *type, PyObject *args,
                                  PyObject *keywds) {
...
static int SM9MasterKey_init(SM9MasterKeyObject *self, PyObject *args,
                             PyObject *keywds) {
  int ok;
  static char *kwlist[] = {NULL};
  // 不接受任何参数
...
static PyObject *SM9MasterKey_generate(PyTypeObject *type,
...
static PyObject *SM9MasterKey_from_der(PyTypeObject *type, PyObject *args,
                                       PyObject *keywds) {
  int ok;
  static char *kwlist[] = {"data", NULL};
  const char *data;
...
static PyObject *SM9MasterKey_to_der(SM9MasterKeyObject *self,
                                     PyObject *Py_UNUSED(args)) {
  // to_der(self) -> bytes
  ...
  if (ret != GMSSL_INNER_OK) {
    PyErr_SetString(GmsslInnerError,
                    "libgmssl inner error in sm9_enc_master_key_to_der");
    return NULL;
  }
  // 这里不用 p ，因为 p 已经变了
...
static PyObject *SM9MasterKey_decrypt_from_der(PyTypeObject *type,
                                               PyObject *args,
                                               PyObject *keywds) {
  int ok;
  static char *kwlist[] = {"password", "data", NULL};
...
static PyObject *SM9MasterKey_encrypt_to_der(SM9MasterKeyObject *self,
                                             PyObject *args, PyObject *keywds) {
  int ok;
  static char *kwlist[] = {"password", NULL};
  const char *password;
  ...
  size_t len = 0;
  int ret =
      sm9_enc_master_key_info_encrypt_to_der(&self->master, password, &p, &len);
  if (ret != GMSSL_INNER_OK) {
    PyErr_SetString(
        GmsslInnerError,
        "libgmssl inner error in sm9_enc_master_key_info_encrypt_to_der");
    return NULL;
  }
  // 这里不用 p ，因为 p 已经变了
  return Py_BuildValue("y#", buf, (Py_ssize_t)len);
...
static PyObject *SM9MasterKey_decrypt_from_pem(PyTypeObject *type,
  ...
  if (ret != GMSSL_INNER_OK) {
    ...
    PyErr_SetString(
        GmsslInnerError,
        "libgmssl inner error in sm9_enc_master_key_info_decrypt_from_pem");
    return NULL;
  }
  fclose(fp);
  return (PyObject *)self;
...
static PyObject *SM9MasterKey_encrypt_to_pem(SM9MasterKeyObject *self,
  ...
  if (ret != GMSSL_INNER_OK) {
    ...
    PyErr_SetString(
        GmsslInnerError,
        "libgmssl inner error in sm9_enc_master_key_info_encrypt_to_pem");
    return NULL;
  }
  fclose(fp);
  Py_RETURN_NONE;
...
static PyObject *SM9MasterKey_extract_key(SM9MasterKeyObject *self,
                                          PyObject *args, PyObject *keywds) {
  int ok;
...
static PyObject *SM9MasterKey_public_key(SM9MasterKeyObject *self,
                                         PyObject *Py_UNUSED(ignored)) {
...
PyTypeObject GmsslextSM9MasterKeyType = {
    PyVarObject_HEAD_INIT(NULL, 0).tp_name = "gmsslext.SM9MasterKey",
